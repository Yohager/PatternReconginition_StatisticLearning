...
class Singular_value_decomposition:
    ...
    def reconstruct_figure(self,init_data,pixel_matrix):
        ...
        #cv2.imwrite('init.png',init_data)
        cv2.imwrite('svd_eig50.png',restored_figure)

    def plot_figure(self,data_1,data_2):
        x_lab = list(range(len(data_1)))
        plt.grid(True)
        plt.plot(x_lab,data_1,label = 'Compression Error',color='b')
        plt.plot(x_lab,data_2,label = 'Compression Ratio',color='r')
        plt.xlabel('Top-k Eigenvalues')
        plt.ylabel('Compression Error')
        plt.title('Compression Error and Ratio Changing with Top-k Eigenvalues')
        plt.legend()
        plt.show()
...
if __name__ == '__main__':
    ...
    for i in range(1,100):
        result = Singular_vd.SVD_main_function(Singular_vd,image_pixel_preprocessing,i) * 255
        error.append(np.sum(image_pixel - result))
    print(error)
    x = np.arange(1,100)
    y = [(243*437) / (i**2 + 680*i) for i in x]
    print(y)
    Singular_vd.plot_figure(Singular_vd,error,y)
